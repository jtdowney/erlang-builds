...
pub fn existing_combos_parses_asset_names_test() {
  let names = [
    ...
    "erlang_28.1-1.trixie_armhf.deb",
    "not-a-package.txt",
  ]
  let combos = detect.existing_packages(names)

  assert set.size(combos) == 3
  assert set.contains(
    combos,
  ...
  assert set.contains(
    combos,
    detect.Package(version: "28.1", codename: "trixie", arch: "armhf"),
  )
}
...
pub fn missing_matrix_skips_built_combos_test() {
  ...
  let distros = [
    detect.Distro(codename: "bookworm", image: "debian:bookworm", arches: [
      "amd64", "arm64", "armhf",
    ]),
  ]
  let existing =
  ...
  assert detect.missing_matrix(latest, distros:, existing:)
    == [
      detect.BuildConfig(
        otp: "28.1",
        codename: "bookworm",
...
pub fn forced_version_matrix_json_test() {
  let config =
    detect.Config(otp_lines: ["28", "29"], distros: [
      detect.Distro(codename: "bookworm", image: "debian:bookworm", arches: [
        "amd64",
      ]),
    ])
...
pub fn autodetect_matrix_json_test() {
  let config =
    detect.Config(otp_lines: ["28", "29"], distros: [
      ...
      detect.Distro(codename: "noble", image: "ubuntu:24.04", arches: [
        "amd64", "arm64",
      ]),
    ])
...
pub fn distributions_conf_test() {
  let config =
    detect.Config(otp_lines: ["28"], distros: [
      detect.Distro(codename: "bookworm", image: "debian:bookworm", arches: [
        "amd64", "arm64", "armhf",
      ]),
      detect.Distro(codename: "noble", image: "ubuntu:24.04", arches: [
